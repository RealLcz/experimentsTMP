#!/usr/bin/env bash
# MoE-Localized RL Pilot: Smoke test for OLMoE + GSPO + MoE-only parameter update.
# Runs a tiny number of steps to verify the pipeline works end-to-end with
# only MoE experts + router trainable.
#
# Usage: sbatch run_smoke_moe.sh   (or bash run_smoke_moe.sh on a GPU node)
set -xeuo pipefail
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-olmoe_moe_smoke}
OUTPUT_DIR=${OUTPUT_DIR:-/mnt/vast/home/ym56kacy/jinhe/MendelGM/wom/outputs/smoke_moe}
PARAM_MODE=${PARAM_MODE:-moe}
########################### end user-adjustable ###########################
...
ACTOR=(
    ...
    # Parameter update policy (localized RL): MoE-only
