...
class RayPPOTrainer:
    ...
    def _compute_old_log_prob(self, batch: DataProto):
        ...
        router_shift_enabled = False
        policy_loss_cfg = self.config.actor_rollout_ref.actor.policy_loss
        if policy_loss_cfg is not None:
            rs_cfg = policy_loss_cfg.get("router_shift", None)
            if rs_cfg is not None:
                router_shift_enabled = rs_cfg.get("enabled", False)
        ...
        batch_td["_capture_router_trace_tensor"] = torch.full(
            batch_td.batch_size, router_shift_enabled, dtype=torch.bool
        )
        output = self.actor_rollout_wg.compute_log_prob(batch_td)
        # gather output
        ...
        router_scores = tu.get(output, "router_scores")
        router_indices = tu.get(output, "router_indices")
        if router_shift_enabled and (router_scores is None or router_indices is None):
            raise RuntimeError(
                "RSPO old-policy router capture was enabled, but the actor worker did not return "
        ...
        if router_scores is not None and router_indices is not None:
            result["old_router_scores"] = no_padding_2_padding(router_scores, batch_td).float()
            result["old_router_indices"] = no_padding_2_padding(router_indices, batch_td)
            result["old_router_scores_nested"] = router_scores
            result["old_router_indices_nested"] = router_indices
        if sum_pi_squared is not None:
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        batch_td["_compute_router_shift_tensor"] = torch.full(
            batch_td.batch_size, router_shift_enabled, dtype=torch.bool
        )
