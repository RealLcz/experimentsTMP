...
class PPOTrainer(ABC):
    ...
    def _compute_old_log_prob(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        ...
        # 1. compute log probs
        router_shift_cfg = self.config.actor_rollout_ref.actor.policy_loss.get("router_shift", None)
        router_shift_enabled = bool(router_shift_cfg and router_shift_cfg.get("enabled", False))
        batch.extra_info.update(
            {
                "calculate_entropy": True,
                "compute_loss": False,
                "temperature": self.config.actor_rollout_ref.rollout.temperature,
                # V1 transports control data through KVBatchMeta.extra_info.  Without
                # this flag the actor forward succeeds but never installs the router
                # hooks, and RSPO only fails much later in the actor loss.
                "capture_router_trace": router_shift_enabled,
                "compute_router_shift": False,
            }
        )
        ...
        if router_shift_enabled:
            fields.extend(["router_scores", "router_indices"])
        ...
        old_policy_fields = ["old_log_probs", "entropy"]
        if router_shift_enabled:
            if "router_scores" not in data or "router_indices" not in data:
                raise RuntimeError(
                    "RSPO old-policy router capture was enabled in the V1 trainer, but "
                    f"TransferQueue returned fields={list(data.keys())}."
                )
            router_scores = data.pop("router_scores")
            router_indices = data.pop("router_indices")
            if not router_scores.is_nested or not router_indices.is_nested:
                raise RuntimeError(
                    "RSPO V1 expects full-sequence nested router traces from the actor; "
                    f"got scores_nested={router_scores.is_nested}, "
                    f"indices_nested={router_indices.is_nested}."
                )
            # Preserve both old traces in their full-sequence representation.  The
            # V1 TransferQueue path may repad/repartition fields independently; doing
            # the response slice here caused the old trace to retain prompt tokens on
            # some ranks.  The actor forward slices old and current traces together,
            # after the final shuffle/dynamic batching, using one shared input layout.
            data["old_router_scores_nested"] = router_scores
            data["old_router_indices_nested"] = router_indices
            old_policy_fields.extend(["old_router_scores_nested", "old_router_indices_nested"])
        batch = tq.kv_batch_put(
            keys=batch.keys, partition_id=batch.partition_id, fields=data.select(*old_policy_fields)
        )

        # Do not let the following reference-policy forward capture and overwrite
        # the old actor traces in the shared TransferQueue partition.
        batch.extra_info["capture_router_trace"] = False

        # Padding the full-sequence expert indices here would transiently
        # materialize a very large [batch, seq, layers, top_k] CPU tensor even
        # though entropy/debug metrics do not consume it.  Leave router traces
        # in TransferQueue and only materialize the small metric fields.
        metric_fields = ["entropy", "response_mask"]
        if self.config.actor_rollout_ref.rollout.calculate_log_probs:
            metric_fields.extend(["responses", "rollout_log_probs", "old_log_probs"])
        data = DataProto(batch=data.select(*metric_fields).to_padded_tensor())
    ...
    def _update_actor(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        ...
        extra_info = {
            ...
            "temperature": self.config.actor_rollout_ref.rollout.temperature,
            "capture_router_trace": False,
            "compute_router_shift": router_shift_enabled,
        }
        batch.extra_info.update(extra_info)
