...
def compute_policy_loss_gspo(
    ...
    config: Optional[ActorConfig] = None,
    rollout_is_weights: torch.Tensor | None = None,
    router_shift_weight: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    ...
    # RSPO: apply router-shift rescaling in log-space before sequence ratio (Eq. 11).
    if router_shift_weight is not None:
        from verl.utils.router_shift.core import apply_router_shift_to_log_ratio

        negative_approx_kl = apply_router_shift_to_log_ratio(negative_approx_kl, router_shift_weight)
