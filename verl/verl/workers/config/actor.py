...
@dataclass
class ParameterUpdateConfig(BaseConfig):
    """Configuration for the parameter update policy (localized RL).

    Controls which parameter subspace of the actor model is trainable. The
    policy is applied after model construction and before FSDP wrapping /
    optimizer creation, so it is compatible with FSDP1 (use_orig_params=False)
    and FSDP2.

    Args:
        mode (str): Which parameters to train.
            - 'full': all parameters trainable (vanilla veRL behavior, default).
            - 'moe': only MoE experts + router trainable.
            - 'experts_only': only MoE experts trainable (router frozen).
            - 'attention_only': only attention layers trainable (control for MoE-only).
        strict (bool): When True, validate that the expected categories are
            trainable / frozen and abort training on any mismatch. No silent
            fallback.
        log_parameter_stats (bool): Print the per-category parameter audit at
            the start of training.
        output_dir (Optional[str]): Directory to write parameter_policy.json.
            If None, the audit is only logged.
    """
    ...
    def __post_init__(self):
        ...
        if self.mode not in valid_modes:
            raise ValueError(
                f"Invalid parameter_update.mode: {self.mode}. Must be one of {valid_modes}"
            )


@dataclass
class RouterShiftConfig(BaseConfig):
    """Configuration for RSPO router-shift rescaling (GSPO+RS plug-in)."""
    ...
    def __post_init__(self):
        if not (0.0 < self.gamma_min <= 1.0):
            raise ValueError(f"router_shift.gamma_min must be in (0, 1], got {self.gamma_min}")
...
class PolicyLossConfig(BaseConfig):
    ...
    router_shift: RouterShiftConfig = field(default_factory=RouterShiftConfig)
...
class ActorConfig(BaseConfig):
    ...
    parameter_update: ParameterUpdateConfig = field(default_factory=ParameterUpdateConfig)
