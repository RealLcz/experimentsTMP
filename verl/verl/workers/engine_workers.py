...
class TrainingWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: TrainingWorkerConfig):
        ...
        self.parameter_update_config = getattr(self.config, "parameter_update_config", None)
        ...
        self.engine: BaseEngine = EngineRegistry.new(
            ...
            parameter_update_config=self.parameter_update_config,
        )
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if "ref" in self.role:
            ...
            # Force-sync strategy into engine_config (same OmegaConf bypass as actor).
            if getattr(ref_config.engine, "strategy", None) != ref_config.strategy:
                logger.warning(
                    "ref.engine.strategy=%s != ref.strategy=%s; forcing %s.",
                    getattr(ref_config.engine, "strategy", None),
                    ref_config.strategy,
                    ref_config.strategy,
                )
                ref_config.engine.strategy = ref_config.strategy
        ...
        if "actor" in self.role:
            ...
            # Force-sync strategy into engine_config. The __post_init__ sync in
            # actor.py does not run under omega_conf_to_dataclass, so engine_config.strategy
            # stays at "fsdp" default and FSDP2 + offload_policy is silently disabled.
            if getattr(actor_config.engine, "strategy", None) != actor_config.strategy:
                logger.warning(
                    "actor.engine.strategy=%s != actor.strategy=%s; forcing %s (OmegaConf bypassed __post_init__).",
                    getattr(actor_config.engine, "strategy", None),
                    actor_config.strategy,
                    actor_config.strategy,
                )
                actor_config.engine.strategy = actor_config.strategy
            ...
            actor_training_config = TrainingWorkerConfig(
                ...
                parameter_update_config=getattr(actor_config, "parameter_update", None),
            )
        ...
        # Stage actor/ref off GPU before colocated vLLM server init when using manual
        # param_offload. Do NOT call to(cpu) under FSDP2 CPUOffloadPolicy — that policy
        # owns placement and an explicit .to(cpu) leaves the module half-moved (#5995).
        if "actor" in self.role:
            engine = self.actor.engine
            if engine.is_param_offload_enabled:
                engine.to("cpu", model=True, optimizer=True, grad=False)
        if "ref" in self.role:
