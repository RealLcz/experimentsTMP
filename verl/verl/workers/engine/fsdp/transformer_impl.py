...
class FSDPEngine(BaseEngine):
    ...
    def __init__(
        ...
        parameter_update_config: Optional[Any] = None,
    ):
        """
        ...
        self.parameter_update_config = parameter_update_config
    ...
    def _build_optimizer(self, module):
        ...
        # allocating optimizer state (exp_avg, exp_avg_sq) for frozen
        # parameters when a parameter_update policy (moe / experts_only) is
        # active. For mode='full' every parameter is trainable so this is a
        # no-op and preserves vanilla veRL behavior.
    ...
    def _build_model_optimizer(self):
        ...
        # Apply parameter update policy (localized RL). Must happen after model
        # load (LoRA/QAT already applied) and BEFORE FSDP wrapping so that
        # requires_grad is set on the original parameters. With FSDP1
        # (use_orig_params=False) parameters are flattened after wrapping, so
        # per-parameter requires_grad must be configured here. For mode='full'
        # (default) this is a no-op and preserves vanilla veRL behavior.
        self._apply_parameter_update_policy(module)
    ...
    def _apply_parameter_update_policy(self, module):
        """Apply the parameter update policy to the model (pre-FSDP).

        No-op when no parameter_update_config is provided (backward compatible).
        """
        cfg = self.parameter_update_config
        if cfg is None:
            return
        ...
        log_parameter_stats = getattr(cfg, "log_parameter_stats", True)
        output_dir = getattr(cfg, "output_dir", None)
        # Only the actor (forward_only=False) needs to freeze params; the ref
        # policy and rollout log-prob engine should keep all params as-is.
        if self.engine_config.forward_only and mode != "full":
            logger.info(
                "[parameter_update] forward_only engine: skipping non-full policy "
                f"(mode={mode}) to preserve reference/eval behavior."
            )
            return
        self.parameter_update_summary = apply_parameter_update_policy(
            module,
            mode=mode,
            strict=strict,
            log_parameter_stats=log_parameter_stats,
...
class FSDPEngineWithLMHead(FSDPEngine):
    ...
    def forward_step(self, micro_batch: TensorDict, loss_function, forward_only):
        ...
        with autocast_ctx:
            ...
            capture_router = tu.get_non_tensor_data(data=micro_batch, key="capture_router_trace", default=False)
            compute_router_shift = tu.get_non_tensor_data(data=micro_batch, key="compute_router_shift", default=False)

            # Scalar NonTensorData is normally preserved by TensorDict/Ray
            # dispatch. Keep a batch-shaped tensor sentinel as well because
            # nested/dynamic batching passes through several independent
            # chunk/index/serialization layers. A lost scalar flag previously
            # made RSPO silently skip both old- and current-policy capture.
            for sentinel_key, current_value in (
                ("_capture_router_trace_tensor", capture_router),
                ("_compute_router_shift_tensor", compute_router_shift),
            ):
                sentinel = micro_batch.get(sentinel_key)
                if sentinel is not None:
                    sentinel = sentinel.reshape(-1)
                    if sentinel.numel() == 0 or not torch.equal(sentinel, sentinel[:1].expand_as(sentinel)):
                        raise RuntimeError(f"RSPO flag sentinel {sentinel_key} is empty or inconsistent")
                    current_value = bool(sentinel[0].item())
                if sentinel_key == "_capture_router_trace_tensor":
                    capture_router = bool(current_value)
                else:
                    compute_router_shift = bool(current_value)
            router_capture = None
            if capture_router or compute_router_shift:
                from verl.utils.router_shift.capture import RouterTraceCapture

                if self._router_capture is None:
                    self._router_capture = RouterTraceCapture()
                router_capture = self._router_capture
                router_capture.clear()
                root_module = getattr(self.module, "module", self.module)
                router_capture.enable(root_module)
            ...
            if router_capture is not None:
                if not use_remove_padding:
                    # SDPA receives a dense, right-padded batch. Gemma4 routers
                    # flatten its first two dimensions and therefore capture
                    # padding rows as well. Strip exactly those rows before the
                    # traces are wrapped with the original jagged offsets.
                    input_ids_nested = micro_batch["input_ids"]
                    seq_lengths = input_ids_nested.offsets().diff()
                    dense_width = model_inputs["input_ids"].shape[1]
                    valid_rows = (
                        torch.arange(dense_width, device=seq_lengths.device).unsqueeze(0)
                        < seq_lengths.unsqueeze(1)
                    ).reshape(-1)
                    row_indices = valid_rows.nonzero(as_tuple=False).squeeze(-1).to(torch.long)
                    router_capture.select_token_rows(row_indices)
                trace = router_capture.get_trace()
                if trace is None:
                    raise RuntimeError(
                        "RSPO router trace capture was requested, but no router forward hook fired. "
                        "The active model execution path does not match the registered router modules."
                    )
                else:
                    input_ids = micro_batch["input_ids"]
                    cu_seqlens = input_ids.offsets()
                    for key in ("router_scores", "router_indices"):
                        v = trace[key]
                        if self.use_ulysses_sp and "pad_size" in output_args:
                            pad_size = output_args["pad_size"]
                            v = gather_outputs_and_unpad(v, gather_dim=0, unpad_dim=0, padding_size=pad_size)
                        model_output[key] = torch.nested.nested_tensor_from_jagged(v, cu_seqlens)

                    if compute_router_shift:
                        missing_old = [
                            key
                            for key in ("old_router_scores_nested", "old_router_indices_nested")
                            if key not in micro_batch
                        ]
                        if missing_old:
                            raise RuntimeError(
                                "RSPO current-policy capture is enabled but the V1/legacy batch is missing "
                                + ", ".join(missing_old)
                            )
                        old_scores_nested = micro_batch["old_router_scores_nested"]
                        old_idx_nested = micro_batch["old_router_indices_nested"]
                        old_idx = old_idx_nested.values()
                        current_scores = router_capture.get_current_scores_at_old_indices(old_idx)
                        if self.use_ulysses_sp and "pad_size" in output_args:
                            pad_size = output_args["pad_size"]
                            current_scores = gather_outputs_and_unpad(
                                current_scores, gather_dim=0, unpad_dim=0, padding_size=pad_size
                            )
                        from verl.workers.utils.padding import no_padding_2_padding

                        current_nested = torch.nested.nested_tensor_from_jagged(current_scores, cu_seqlens)
                        # Slice old and current with the exact same post-shuffle
                        # prompts/responses.  This guarantees response alignment even
                        # when TransferQueue has repartitioned jagged tensors.
                        micro_batch["old_router_scores"] = no_padding_2_padding(old_scores_nested, micro_batch)
                        micro_batch["old_router_indices"] = no_padding_2_padding(old_idx_nested, micro_batch)
                        model_output["current_router_scores"] = no_padding_2_padding(current_nested, micro_batch)
